...
def create_engine_with_retry(max_retries: int = MAX_RETRIES) -> any:
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            last_error = e
            logger.warning(f"Database connection attempt {attempt}/{max_retries} failed: {e}", extra={"attempt": attempt, "max_retries": max_retries})
            if attempt < max_retries:
                logger.info(f"Retrying in {delay} seconds...")
                time.sleep(delay)
                delay *= RETRY_BACKOFF
            else:
                logger.error(f"Failed to connect to database after {max_retries} attempts", exc_info=True)
    raise OperationalError(f"Failed to connect to database after {max_retries} attempts", params=None, orig=last_error)
